...
function asString(value) {
  ...
  if (typeof value === "object") return value.name || value.title || value.slug || value.imperial || (value.year != null ? String(value.year) : null) || null;
  return null;
}
...
const GP_KEYS = ["GP", "gp", "gamesPlayed", "games"];
const GOALS_KEYS = ["G", "goals", "g"];
const ASSISTS_KEYS = ["A", "assists", "a"];
const POINTS_KEYS = ["PTS", "points", "tp", "pts"];
const PIM_KEYS = ["PIM", "pim", "penaltyMinutes"];
const PLUS_MINUS_KEYS = ["PM", "plusMinus", "plus_minus", "pm", "plusMinusRating"];
...
function normalizeStatsRow(row) {
  ...
  return {
    ...
    gp: toNumber(firstDefined(stats, GP_KEYS)),
    goals: toNumber(firstDefined(stats, GOALS_KEYS)),
    assists: toNumber(firstDefined(stats, ASSISTS_KEYS)),
    points: toNumber(firstDefined(stats, POINTS_KEYS)),
    pim: toNumber(firstDefined(stats, PIM_KEYS)),
    plusMinus: toNumber(firstDefined(stats, PLUS_MINUS_KEYS)),
...
function looksLikeStatsRow(row) {
  const stats = row.regularStats || row.stats || row;
  return Boolean(row.season && (firstDefined(stats, GP_KEYS) != null));
}

const EMPTY_RECENT_FORM = { spanGames: null, gp: null, goals: null, assists: null, points: null, plusMinus: null };
const EMPTY_CONTEXT = { season: null, team: null, league: null };

// Elite Prospects embeds `latestGameLogsAggregated`: rolling last-N-games
// totals (e.g. limit: 5, limit: 10). These are NOT season totals and must
// never be stored as `latestSeason` — they are a separate "recent form"
// signal. We pick the largest valid (GP > 0) window as most representative.
function selectRecentForm(player) {
  const entries = Array.isArray(player?.latestGameLogsAggregated) ? player.latestGameLogsAggregated : [];
  const candidates = entries
    .map(entry => ({
      limit: typeof entry?.limit === "number" ? entry.limit : null,
      gp: toNumber(firstDefined(entry?.stats || {}, GP_KEYS)),
      goals: toNumber(firstDefined(entry?.stats || {}, GOALS_KEYS)),
      assists: toNumber(firstDefined(entry?.stats || {}, ASSISTS_KEYS)),
      points: toNumber(firstDefined(entry?.stats || {}, POINTS_KEYS)),
      plusMinus: toNumber(firstDefined(entry?.stats || {}, PLUS_MINUS_KEYS))
    }))
    .filter(entry => typeof entry.limit === "number" && typeof entry.gp === "number" && entry.gp > 0)
    .sort((a, b) => b.limit - a.limit);

  if (!candidates.length) return { ...EMPTY_RECENT_FORM };
  const best = candidates[0];
  return {
    spanGames: best.limit,
    gp: best.gp,
    goals: best.goals,
...
// `latestStats` gives the player's current team/league/season context, but
// carries no game counts — it must never be presented as a season total.
function selectCurrentContext(player) {
  const latestStats = player?.latestStats || {};
  return {
    season: asString(latestStats.season) || null,
    team: asString(firstDefined(latestStats, ["teamName", "team"])) || null,
    league: asString(firstDefined(latestStats, ["leagueName", "league"])) || null
  };
}
...
function parseProfileHtml(html) {
  ...
  return {
    ...
    recentForm: selectRecentForm(bio),
    currentContext: selectCurrentContext(bio)
  };
}
...
async function fetchEliteProspectsData(profileUrl) {
  ...
  const { bio, latestSeason, recentForm, currentContext } = parseProfileHtml(html);
  ...
  return {
    ...
    latestSeason,
    recentForm: recentForm || { ...EMPTY_RECENT_FORM },
    currentContext: currentContext || { ...EMPTY_CONTEXT },
